...
#import "ECPieChart.h"
#import "ECPieItemsView.h"
#import "ECPieForeBGView.h"
#import "ECShowInfoView.h"
...
@interface ECPieChart ()

@property (nonatomic,strong) ECPieForeBGView * pieForeView;
...
@property (nonatomic,strong) ECShowInfoView * showInfoView;

@end

@implementation ECPieChart
...
- (void)showAnimation
{
    ...
    if (_valueArr.count>0) {
        ...
        for (NSInteger i = 0; i<_countPreAngeleArr.count-1; i++) {
            ...
            ECPieItemsView *itemsView = [[ECPieItemsView alloc] initWithFrame:CGRectMake(10, 10, wid/2, wid/2) andBeginAngle:[_countPreAngeleArr[i] floatValue] andEndAngle:[_countPreAngeleArr[i+1] floatValue] andFillColor:colors[i%colors.count]];
        ...
        _pieForeView = [[ECPieForeBGView alloc] initWithFrame:CGRectMake(10, 10, wid,  wid)];
        ...
        if (_showInfoView==nil) {
            _showInfoView = [[ECShowInfoView alloc] init];
...
- (void)judgeWhitchOneIsNowAngle:(CGFloat )angel andShowPoint:(CGPoint)p
{
    ...
    for (NSInteger i = 0; i<_countPreAngeleArr.count-1; i++) {
        ...
        if ([_countPreAngeleArr[i+1] floatValue]>=angel) {
            ...
            ECPieItemsView *itemsView = _layersArr[i];
            
            ECPieItemsView *saveItems = _layersArr[_saveIndex];
